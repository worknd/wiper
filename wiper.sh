...
HDPARM=$(find_prog /opt/sbin/hdparm)	|| exit 1
...
##
HDPVER=$($HDPARM --verbose -V | $GAWK '{gsub("[^0-9.]","",$2); if ($2 > 0) print ($2 * 100); else print 0; exit(0)}')
...
function get_devpath(){
	dir="$1"
	kdev=$($STAT -c "%04D" "$dir" 2>/dev/null)
	[ "$kdev" = "" ] && exit 1
	major=$((0x${kdev:0:2}))
	minor=$((0x${kdev:2:2}))
...
if [ "$method" = "online" ]; then
	...
	## So, reserve at least 0.01% or 4096 KB, whichever is larger:
	##
...
if [ "$method" = "online" ]; then
	...
	if ! $HDPARM --fallocate "${tmpsize}" $tmpfile &>/dev/null ; then
		FALLOCATE=$(find_prog /opt/bin/fallocate) || exit 1
		if ! $FALLOCATE -l "${tmpsize}K" $tmpfile &>/dev/null ; then
